...
Routa does not solve application security.

Routa makes security configuration visible, typed, consistently wired, and documented at the API boundary. The application still owns credential validation, authorization policy decisions, sensitive business-flow protection, secret management, and security review.
...
Routa has one request execution pipeline: middleware.
...
Initial guardrail implementation can live in Routa's own config and route analysis step. Integration with external linters or custom lint rules is optional and should be evaluated during implementation planning.

## Part 2.1: Authentication Modes and Middleware Model

### Decisions

- Routa can expose configuration and integration points for multiple authentication modes:
  - Bearer token
  - API key
  - Session/cookie
- Each mode is optional and can be enabled independently.
- Routa uses a two-layer authentication flow:
...
- Routa can support two auth provider modes:
  - `better-auth`: Routa provides a configurable first-party integration powered by Better Auth.
  - `custom`: application defines its own auth structure against Routa's provider contract.
- Routa defines a stable auth provider contract so framework middleware behavior remains consistent across providers.
- Better Auth integration is the default scaffolded implementation path, not a hard lock-in.
- In Better Auth mode, Routa may use Better Auth support/plugins for session/cookie, bearer token, and API key authentication where selected by the developer.
- Routa should provide an auth setup generator command (`npx routa auth`) to scaffold provider selection, auth mode choices, and middleware wiring.
...
- Routa provides a first-party Better Auth integration as the default scaffolded implementation path.
- Routa's framework contract remains provider-neutral.
- Better Auth and custom providers both plug into Routa's standard auth-resolution middleware.
- Custom auth should normally be implemented through Routa's auth provider and credential strategy interfaces, not by replacing the auth pipeline with ad hoc middleware.
- Credential strategies should cover enabled auth modes such as bearer token, API key, and session/cookie.
- Routa's auth-resolution middleware owns credential detection, priority selection, validation of the selected credential, normalized `ctx.state.auth`, standard auth errors, and security events.
- Developers may still write custom auth middleware as an escape hatch, but this is not the recommended path because it bypasses Routa's shared priority config, normalized auth context, standard error mapping, provider swap path, and future OpenAPI/security metadata integration.

### Credential Selection

- Routa uses a configurable credential priority model.
- Default priority is API-oriented: bearer token, API key, then session/cookie.
- If multiple credentials are present, Routa selects the first presented enabled credential by priority and validates only that credential.
- If the selected credential is invalid, Routa returns `401 Unauthorized` and does not fall back to lower-priority credentials.
- If no credential is present, Routa attaches anonymous auth context unless a later guard requires authentication.
...
- `ctx.state.auth` is the only Routa-standard auth context.
...
```ts
type AuthContext<User = RoutaAuthUser, Session = RoutaAuthSession> =
	| { status: "anonymous" }
	| {
...
- Routa can make authorization declarations visible at three levels:
...
- Routa should provide a guardrail warning for repetitive middleware chains where `requirePermission(action)` is immediately followed by `authorizeResource(action, ...)`; projects may escalate that warning to an error through guardrail configuration.
...
- Field-level authorization is modeled through named response variants, not a manual `oneOf` success schema. Routa keeps runtime variants explicit and generates OpenAPI `oneOf` documentation automatically when same-status variants expose different schemas for the same media type.
- Routa does not provide a public `oneOf` route response helper in v1. Developers should model different successful shapes as named response variants. Other composition helpers remain available where they do not weaken the response variant model.
...
- Named response variants preserve Routa's `{ type, data }` runtime model while still allowing OpenAPI to document a single `200` response with generated `oneOf`.
...
- In the Hono runtime adapter, Routa may implement rate limiting by composing a Hono-compatible rate limiter. The adapter must preserve Routa semantics.
...
- Developer-facing rate-limit keys are identity dimensions only:
  - `ip`
  - `auth`
- Routa always includes the normalized route id in the internal rate-limit bucket namespace. Developers do not configure `route` as a key because route scope is implied by where the rate limit is declared or inherited.
...
- Default `429 Too Many Requests` behavior includes `Retry-After` and Routa standard error body/code.
...
- Routa should provide first-party store adapters for local memory and Redis, and support custom adapters for other backends.
- In multi-instance deployments without a distributed store, Routa should emit a startup warning.
...
- The Hono adapter can reuse mature Hono-compatible middleware without exposing Hono-specific rate-limit APIs as Routa's public contract.
...
- Routa uses environment-aware defaults: lax for development speed, strict for production safety.
- Routa uses preset-first security configuration with explicit overrides for advanced users.
- Production defaults:
...
- Routa should not apply an aggressive default `Content-Security-Policy` to API routes in v1 because APIs usually return data, not HTML, and CSP is application-specific.
- Routa should not use deprecated browser headers such as `X-XSS-Protection`.
...
- `https.onInsecureRequest` may support:
  ...
  - `redirect`
  - `warn`
- Routa determines public HTTPS status from the direct request unless trusted proxy support is enabled.
...
- When trusted proxy is enabled, Routa may use `X-Forwarded-Proto` to determine public HTTPS and `X-Forwarded-For` to determine client IP for `ctx.request.ip`, rate limits, and security logs.
...
- Routa provides SSRF protection for Routa-managed outbound helpers, not arbitrary direct runtime `fetch` calls.
- Routa should not monkey-patch, ban, or replace global `fetch`.
- Routa provides a recommended `safeFetch` helper for user-input URLs and third-party outbound HTTP.
- `safeFetch` uses framework-managed outbound security checks and standard Routa error behavior.
- Native `fetch` remains available as an escape hatch, but direct use is outside Routa's SSRF protection boundary.
...
- Guardrails should warn when routes accept URL input but do not use Routa-managed outbound helpers, when outbound protections are disabled in production, or when sensitive routes perform outbound HTTP without explicit origins.

### Rationale

- Defines a clear security boundary: Routa can protect `safeFetch`, but cannot prove arbitrary user `fetch` calls are safe.
...
This proposal applies when a developer chooses Routa's default scaffolded auth path.
If the developer chooses custom auth, they should still implement Routa's provider and credential strategy contracts so the standard auth-resolution middleware can run the same flow.

### Mode A: `better-auth` (Opt-in implementation path)

When selected, Routa should provide a first-party integration aligned with common industry standards:
...
#### What Routa should generate in this mode

- `npx routa auth` scaffolds Better Auth wiring, provider config, credential mode choices, middleware registration, and starter policy callbacks.
...
If Better Auth is not selected, Routa does not impose a concrete auth implementation.
Routa expects the app to implement framework provider and credential strategy interfaces.
The recommended custom path still uses Routa's standard auth-resolution middleware instead of replacing the auth pipeline with ad hoc middleware.
...
3. **Auth Context Normalizer**
   - Responsibility: map provider-specific results into Routa's normalized `ctx.state.auth` shape

4. **Authorization Policy Callbacks**
   - Responsibility: evaluate action/resource access (`allow`/`deny`) when called by Routa guard middleware such as `authorizeResource`

5. **Security Event Emitter**
   - Responsibility: emit structured auth/audit events to app logger/telemetry

6. **Error Mapper**
   - Responsibility: map auth failures to Routa's standard error shape and HTTP semantics
...
- Better Auth is the default scaffolded implementation path, but Routa's route and middleware contracts remain provider-neutral.
- Custom auth should normally implement Routa provider/credential strategy interfaces so credential priority, normalized auth context, standard errors, security events, and future OpenAPI metadata stay consistent.
- Routa has one request pipeline: middleware. Guards are middleware; hooks are callbacks invoked by framework-controlled code; guardrails are diagnostics.
- Rate-limit identity keys exposed to developers are only `ip` and `auth`; Routa includes the normalized route id internally.
- Field-level authorization should use named same-status response variants. Routa generates OpenAPI `oneOf` for documentation when needed, but does not expose a public `oneOf` route response helper in v1.
- `safeFetch` protection applies only to Routa-managed outbound HTTP. Native `fetch` remains an escape hatch outside Routa's SSRF protection boundary.
