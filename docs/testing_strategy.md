# Routa Testing Strategy

## Purpose

This document defines how Routa itself should be tested.

It is not an acceptance spec and it is not the user-facing test-helper design. Specs describe expected behavior. Test helpers describe APIs Routa may give application developers. This file describes the internal validation matrix needed to prove Routa works.

## Core Rule

Test each Routa promise at the layer where it can actually fail.

Routa is not only a runtime library. It is also:
...
Runtime request tests alone are not enough because Routa's core value includes compile-time inference, stable generated source, framework-aware diagnostics, and OpenAPI contract alignment.
...
Use focused graph tests for Routa's compiler-like validation layer.
...
- stale or inconsistent `.routa/routes.gen.ts` detection
...
- generated `.routa/manifest.json`
- generated `.routa/openapi-baseline.json`
- generated `.routa/routes.gen.ts`
- stable imports
- stable ordering
- stable formatting
...
```txt
run routa scaffold input/openapi.yaml
...
- `routa check` exit codes
- `routa build` stops before emit on graph validation failure
...
```txt
routa check exits non-zero on duplicate GET /users
routa build exits before emit when graph validation fails
routa scaffold openapi.txt fails before creating files
```
...
Routa's diagnostics are product behavior, not incidental logs. Tests should keep them stable enough for users and CI.
...
- unsupported production defaults where Routa defines guardrails

These are not route behavior tests. They prove Routa reports unsafe or inconsistent configuration at check, startup, or build boundaries.
...
- CLI tests for `routa check`, `routa build`, and invalid scaffold input
- OpenAPI drift/baseline tests
- Hono runtime request tests for validation, response mapping, and middleware rejects
- diagnostic snapshots for the main Routa graph errors

## Relationship To User-Facing Test Helpers

This strategy is about Routa's internal test suite.
...
Routa may use similar infrastructure internally, but the responsibilities are different:

- internal tests prove Routa's framework promises
- user-facing helpers help applications test their own route behavior

## Review

The core testing idea is correct: Routa needs a layered matrix because its promises fail at different layers.

The handoff's original layers are necessary but not quite complete. The missing groups are:

- contract graph unit tests, because graph construction is the reusable core behind `routa check`, `routa build`, generated metadata, and route-specific ctx
- diagnostic snapshot tests, because framework-aware diagnostics are part of the product value
- regeneration safety tests, because Routa's OpenAPI-to-source loop is only trustworthy if it preserves user work
- configuration and guardrail tests, because many Routa failures should happen before request handling

With those additions, the strategy covers the actual shape of Routa: types, graph validation, generation, CLI behavior, OpenAPI alignment, runtime behavior, file safety, and guardrails.
