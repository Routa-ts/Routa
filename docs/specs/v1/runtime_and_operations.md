...
Routa provides operational boundary behavior such as request identity, structured logs, health/readiness, env validation, and safe defaults.

## Acceptance Cases

```yaml
case: v1_request_id_generated
intent: every request has a Routa-generated request id
input:
  request: GET /users
