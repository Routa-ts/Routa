...
Routa does not generate services or own application business logic.
...
- Routa supports both dependency styles:
...
- Applications may use plain imports without defining services in Routa config.
- If an application uses plain imports, mocking and test precision are application-owned.
- If an application uses Routa services, Routa test helpers may override services per test.
...
- If request-time service creation fails, Routa returns the standard `500 internalError`.
...
Applications may extend the service types. Routa should not require class containers, decorators, or global registries for normal service authoring.
...
- Routa should freeze service registry objects where practical.
- Routa should not deep-freeze service instances.
...
- Routa does not support runtime service replacement outside test helpers.
...
- Routa service overrides are test-only.
...
- Plain imports keep Routa simple for small apps and teams that do not want DI.
...
- Routa should keep an internal schema boundary so future adapters remain possible.
- Routa does not expose a public replaceable validator system in v1.
- The v1 serializer model follows the response representation decisions from Group 1.
- Routa supports built-in approved representation helpers in v1.
- Custom arbitrary content-type serializer registries are deferred.
- Routa has a framework-owned internal metadata registry.
- The metadata registry is read-only for Routa CLI and framework tools in v1.
...
- Routa should avoid scattering direct Zod coupling through every internal subsystem.
...
- Routa's response builder validates the canonical payload first.
- Routa then serializes the selected representation based on the request `Accept` header and the response variant's declared `content` map.
...
- Routa does not guess structured non-JSON layouts from schemas.
- Route handlers return canonical payloads and do not choose the response representation in the normal path. Application services may return canonical payloads if the developer chooses, but Routa does not require that structure.
...
Routa's internal metadata registry is the collected route graph and contract model.
...
- Routa should not rewrite user route files to insert generated type aliases.
...
Routa can borrow the type-safety principle from TanStack Router/Start without copying the frontend model.
...
- Zod gives Routa a strong v1 foundation for inference, validation, and OpenAPI.
...
- A framework-owned metadata registry gives Routa one source for docs, checks, tests, and future tooling.
...
- Routa v1 does not provide a plugin or module system.
- Routa uses CLI integrations for setup-heavy ecosystem features.
- Integrations generate app-owned, editable files.
- Integrations use normal Routa APIs after generation.
- Integrations may install required npm/runtime packages.
- Third-party plugin APIs are deferred.
- Routa should not promise an npm plugin ecosystem in v1.
...
If conflicts are found, Routa should stop before installing packages or writing files unless the user explicitly chooses a conflict handling path.
...
Routa should use the same package manager implied by the command invocation when known:
...
Routa should also inspect project files when command source is ambiguous:
...
If command source and project lockfile disagree, Routa should warn and ask before installing.
...
- If package installation fails, Routa should not write generated files by default.
...
- Hidden plugin mutation conflicts with Routa's schema-first and type-safe direction.
...
- `npx routa testing` installs test dependencies and generates setup files.
- The testing integration defaults to setup only.
- The CLI should ask whether to generate example tests.
- Routa wraps the v0 Hono testing surface instead of exposing Hono as the public testing API.
- Routa test helpers should run HTTP integration tests in memory without starting a real network server by default.
- Future runtime adapters may provide their own internal test harness implementation without changing Routa's public testing API.
...
- Routa does not provide OpenAPI assertion helpers in v1.
...
Routa should provide a route handler unit helper.
...
Routa should provide a middleware unit helper.
...
Routa should provide an in-memory app test harness.

In v1, the harness may use Hono's `app.request()` internally. Hono supports sending a request directly to an app and receiving a standard `Response`, which fits Routa's no-server default.

Routa's public helper should remain adapter-neutral:
...
Routa may also support real server tests later, but the default v1 path should be in-memory.
...
- If users use plain imports instead of Routa services, Routa cannot mock those automatically.
...
Routa should not add a separate OpenAPI assertion helper in v1. This avoids splitting contract testing across both CLI workflows and unit-test helpers.

### Error Snapshot Helper

Routa should provide a helper for standard framework error responses.
...
await expectRoutaError(res).toMatchErrorSnapshot();
```

### Middleware Ordering Helper

Routa should provide test-only middleware tracing.
...
- Vitest gives Routa a modern default test runner without forcing users to design setup from scratch.
- Hono already supports in-memory request testing, so Routa should reuse that internally for the v1 adapter.
- Wrapping Hono keeps Routa's public testing API stable if future adapters such as Express or Fastify are added.
...
- `routa testing` sets up the default test runner and core Routa test helpers.
- `routa security testing` generates optional security regression example tests.
...
The command should inspect the project and generate examples that match configured Routa features.
...
If Routa cannot safely generate a specific example, it should explain why and skip that example rather than generating misleading tests.
...
Routa should not create a second generated API documentation system in v1.
...
```txt
Where is this route defined, and what Routa metadata applies to it?
```
...
```txt
routa routes --format json
routa routes --format markdown
routa routes --format yaml
```

Optional file output:

```txt
routa routes --format markdown --output docs/routes.md
```
...
Routa should provide an error catalog for framework-managed errors.
