...
<Aside type="caution" title="v0 status">
	OpenAPI generation, drift checks, breaking-change checks, and scaffolding for the
	supported JSON contract surface are implemented in v0. Non-JSON content and OpenAPI
	features outside the supported scaffolding list are deferred.
</Aside>
