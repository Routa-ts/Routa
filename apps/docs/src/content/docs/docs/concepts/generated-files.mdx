...
<Aside type="caution" title="v0 status">
	In v0, commit `routes.gen.ts`, `manifest.json`, and `openapi-baseline.json`. After you
	change source, refresh generated metadata; Routa does not run a remote metadata service.
</Aside>
