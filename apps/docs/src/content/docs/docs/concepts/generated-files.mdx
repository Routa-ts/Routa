...
detect removed operations and newly required inputs. It also supplies `info`, `servers`,
and `components.schemas` to generation, so those sections are edited here rather than
derived from source.
...
<Aside type="caution" title="v0 status">
	Tracking and committing `routes.gen.ts`, `manifest.json`, and
	`openapi-baseline.json` is implemented in v0. Generated metadata still has to be
	refreshed after source changes; Routa does not run a remote metadata service.
</Aside>
