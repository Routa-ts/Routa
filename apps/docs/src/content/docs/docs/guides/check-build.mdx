...
<Aside type="note" title="Checks touch the working tree">
	`check`, `generate`, `build`, and `routes` all refresh `.routa/routes.gen.ts` after a
	clean validation. This is intentional, and it is why the CI snippet above compares the
	diff instead of assuming the commands are read-only.
</Aside>
