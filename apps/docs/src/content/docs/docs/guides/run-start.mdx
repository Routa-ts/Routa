...
<Aside type="caution" title="v0 status">
	Development supervision and the built `start` runtime are implemented in v0. `routa
	dev` runs TypeScript through `tsx` and supervises a restart loop, so production
	deployments should build first and run `routa start`.
</Aside>
