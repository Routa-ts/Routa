...
| Problem | Fix |
| --- | --- |
| `Missing .routa/openapi-baseline.json. Run routa scaffold first.` | The project has no accepted contract. Scaffold from a document, or create the baseline with `--update-baseline`. |
...
<Aside type="caution" title="Update the baseline deliberately">
	`--update-baseline` overwrites the accepted contract with whatever the source currently
	produces, including changes nobody reviewed. Run it as its own reviewed commit.
</Aside>
