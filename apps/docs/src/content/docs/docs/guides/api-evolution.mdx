...
<Aside type="caution" title="v0 status">
	In v0, you can use breaking-change checks, deprecation metadata, replacement validation,
	and lifecycle headers. `--update-baseline` overwrites the accepted contract with the
	current source, so run it only as its own reviewed commit.
</Aside>
