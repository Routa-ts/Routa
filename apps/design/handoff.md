# Handoff: Routa landing pages (clean start)
...
| Visual system      | Strong brand color + distinctive type; coherent dark _or_ austere          |
...
- **Express** — Slogan-as-H1 + WebGL atmosphere; docs shell _is_ marketing chrome; austere classic
...
- Bidirectional: edit Zod _or_ OpenAPI, regenerate the other
...
2. Each must use **Lattice** tokens and Routa’s router/contract story — different _composition and metaphor_, not five skins of one layout.
3. Add a small hub at `/landings/` listing all five for review.
4. Wire sidebar links in `apps/design/astro.config.mjs`.
5. Keep production `apps/docs` `/` as a minimal stub (or leave untouched) until a landing is chosen.
6. `pnpm --filter @routa-ts/design build` must pass.
