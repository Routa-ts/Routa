...
Trail resolves global, group, resource segment, route-file, and method middleware into route-specific pipelines and generates route metadata for typechecking.

The ctx type belongs to the resolved route. Trail should not use one giant global `AppCtx` that gives every route fields only provided by protected or admin middleware.
...
- Trail validates middleware order through the route graph.
- Trail generates `.trail/routes.gen.ts` with resolved route metadata.
...
expected:
  behavior:
    - Trail reports invalid middleware order
...
intent: .trail/routes.gen.ts captures route-specific middleware pipeline
input:
