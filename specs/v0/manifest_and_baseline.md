...
Trail tracks generated files and OpenAPI state with committed metadata.
...
input:
  command: trail scaffold openapi.yaml
...
input:
  files:
    .trail/manifest.json: missing
action: trail scaffold openapi.yaml on existing project
expected:
  behavior:
